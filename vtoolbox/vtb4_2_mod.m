function [x,v,t]=vtb4_2_mod(M,K,x0,v0,tf,plotpar)
...
if plotpar~= 0
   if plotpar~=2
       for i=1:N
           ...
           xlabel('time (sec)')
           title(['Response Plot that follows Initial Displacement (X',num2str(i),')'])
           hold on
           grid on
       end
   end 
   if plotpar~=1
       for i=1:N
           ...
           xlabel('time (sec)')
           title(['Response Plot that follows Initial Velocity (X',num2str(i),')'])
           hold on
           grid on
      end
   end
